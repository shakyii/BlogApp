      ...
      <section class="article-section">       
        ...
        <?php 
          if ( isset($_SESSION['logged_in']) && ($_SESSION['typ']=="u" || $_SESSION['typ']=="m") ) {
            echo '<form action="new_post_script.php" method="POST"  enctype="multipart/form-data">
                  <div class="title-box"><label for="title">Tytuł:</label>
                  <input type="text" class="input" placeholder="Tytuł" name="title"></div>
                  <div class="article-text-box"><label for="article">Artykuł:</label>
                  <textarea class="input" rows="15" name="article"></textarea></div>
                  <div class="image-box"><label for="image">Select image to upload:</label>
                  <input type="file" name="img" class="btn img-load"></div>
